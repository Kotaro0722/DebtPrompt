...
| -- | ------ | -------- | ------ | ------- |
| メッセージ ID | 債務者 | 債権者 | 金額 | 返済済みか |

- id は【メッセージ id】を使用する
- 金額が負の値の場合は、定額の返済をしたことを表す

【total】
| id | message_id | debt_id |
| ---------- | ------------------------ | ------------------ |
|自動追加の ID|合計を表示したメッセージの ID|各借金メッセージの ID|
...
- 引数:メッセージの id,借金のメッセージの id(複数)
- 戻り値:なし
- 処理:
  - total にデータを追加する
  - このテーブルに【合計を表示したメッセージの id】【借金のメッセージの id】を追加する
